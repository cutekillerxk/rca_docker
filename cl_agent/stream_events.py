...
# 导入 LangChain Agent
from cl_agent.agent import create_agent_instance, export_to_word, export_to_pdf
        ...
        def respond(message, chat_history, selected_model):
            ...
            try:
                ...
                if not response or not response.strip():
                    ...
                    # Agent直接输出对话格式的文本，无需额外转换
                

                last_agent_response = response  # 保存最后一次回复
                
                # 保存 Agent 响应到 return 文件夹
                try:
                    # 创建 return 文件夹（如果不存在）
                    return_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "return")
                    os.makedirs(return_dir, exist_ok=True)
                    
                    # 生成文件名（格式：agent_response_YYYYMMDD_HHMMSS.txt）
                    ...
                    content_lines.append("Agent 响应记录")
                    content_lines.append("="*70)
                    content_lines.append(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                    content_lines.append(f"用户输入: {message}")
                    content_lines.append("")
                    content_lines.append("-"*70)
                    content_lines.append("完整消息历史:")
                    content_lines.append("-"*70)
                    ...
                    if "messages" in result:
                        for i, msg in enumerate(result["messages"]):
                            ...
                            content_lines.append(f"类型: {msg_type}")
                            
                            # 处理不同类型的消息
                            if msg_type in ["human", "user"]:
                                content_lines.append(f"内容: {msg_content if msg_content else str(msg)}")
                                ...
                                if tool_calls_in_msg:
                                    content_lines.append("工具调用:")
                                    for tool_call in tool_calls_in_msg:
                                        tool_name = tool_call.get("name", tool_call.get("function", {}).get("name", "unknown"))
                                        tool_args = tool_call.get("args", tool_call.get("function", {}).get("arguments", {}))
                                        content_lines.append(f"  - 工具: {tool_name}")
                                        if tool_args:
                                            args_str = json.dumps(tool_args, ensure_ascii=False, indent=2)
                                            content_lines.append(f"    参数: {args_str}")
                                else:
                                    content_lines.append(f"内容: {msg_content if msg_content else str(msg)}")
                                ...
                                content_lines.append(f"工具: {tool_name}")
                                content_lines.append(f"结果: {msg_content if msg_content else str(msg)}")
                    ...
                    print(f"[INFO] Agent响应已保存到: {filepath}")
                    
                except Exception as e:
                    print(f"[WARNING] 保存Agent响应失败: {e}")
                    import traceback
                    traceback.print_exc()
