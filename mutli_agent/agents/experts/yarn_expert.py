...
class YARNExpertAgent(BaseAgent):
    ...
    def __init__(self, llm_client, tools: Optional[Dict[str, Any]] = None):
        ...
        if tools:
            ...
            for tool_name in yarn_tool_names:
                if tool_name in tools:
                    yarn_tools[tool_name] = tools[tool_name]
...
{{
  "action": "call_tool",
  "tool": "get_node_log",
  "args": {{"node_name": "resourcemanager"}}
}}
    ...
    def build_prompt(self, input_data: Dict[str, Any]) -> str:
        ...
        if "logs" in input_data and input_data["logs"]:
            ...
            if isinstance(input_data["logs"], dict):
                # 优先显示YARN相关节点的日志
                yarn_nodes = ["resourcemanager", "nodemanager"]
                ...
                # 显示其他节点（最多2个）
                other_nodes = [n for n in input_data["logs"].keys() if n not in yarn_nodes][:2]
        ...
        if "metrics" in input_data and input_data["metrics"]:
            ...
            # YARN相关指标（如果有）
            if "resourcemanager" in metrics:
                prompt_parts.append("ResourceManager指标：")
                # 可以添加具体的YARN指标
            prompt_parts.append("")
